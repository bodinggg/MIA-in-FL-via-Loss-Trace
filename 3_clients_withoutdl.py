    ...
    for epoch in range(epochs):
        ...
        for data, target in data_loader:
            ...
            for i in range(output.shape[0]):
                ...
                if flag == 1:
                    temp_attack_loss.append(torch.tensor(l_3))
                if flag == 2:
                    temp_non_mem_loss.append(torch.tensor(l_3))
                if flag == 3:
                    temp_target_loss.append(torch.tensor(l_3))
            loss.backward()
            optimizer.step()
            loss_temp += loss.item()
        ...
        if flag == 1:
            ...
            print(f'temp_attack_loss mean is {temp_attack_loss.mean()}')
            attack_loss.append(temp_attack_loss)
        if flag == 2:
            temp_non_mem_loss = torch.stack(temp_non_mem_loss)
            print(f'temp_non_mem_loss mean is {temp_non_mem_loss.mean()}')
...
def fedavg(client_data_loaders, global_model, round, local_epochs):
    ...
    attack_data_loader = client_data_loaders[0]
    non_mem_data_loader = client_data_loaders[4]
    # 获取target_data_loader
    target_data_loader_mem = client_data_loaders[1]

    target_data_loader_nmem = client_data_loaders[4]
    flag = 0    # 判断当前客户端行为
    if shuju == 'cifar10':
    ...
    # 定义客户端的优化器
    client_optimizer = optim.SGD(client_model.parameters(), lr=0.001, momentum=0)
    ...
    #第一阶段结束，接下来直接对客户端C投入“目标”数据集，进行训练，获得模型参数。
    client_accuracy = train(flag, client_model, target_data_loader_nmem, criterion, round,local_epochs, client_optimizer, flag)
...
label_target_mem = torch.ones(target_loss.shape[0], 1)


data_target = target_loss
label_target = label_target_mem
